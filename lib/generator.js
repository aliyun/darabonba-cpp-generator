...
class Visitor {
  ...
  visit(ast, level = 0) {
    ...
    // 将 models 和 exceptions 放在产品版本目录下，避免多 SDK 使用时的路径冲突
    const baseHeaderPath = this.header.replace(/\.h(pp)?$/, '');
    this.modelHeader = baseHeaderPath + '/models';
    this.exceptionHeader = baseHeaderPath + '/exceptions';
  ...
  async visitCMake() {
    ...
    CMakeContent = CMakeContent
      ...
      .replace(/<% source %>/g, this.srcFiles.map((file, index) => index === 0 ? file : `    ${file}`).join('\n'))
  ...
  visitIncludeException(ast, level) {
    ...
    if (!hasFields) {
      ...
      this.visitFieldsToJson(ast.exceptionBody, level + 1);
    }
    this.emit('};\n', level + 1);
  ...
  visitType(ast, level) {
    if (ast.type === 'array') {
      ...
      const moduleName = _name(ast);
      const moduleInfo = this.imports[moduleName];
      if (moduleInfo && moduleInfo.header) {
        this.headers.push(moduleInfo.header);
      }
  ...
  visitNestedModel(ast, level, parentClassName) {
    ...
    // 检查是否有字段
    const hasFields = ast.modelBody.nodes && ast.modelBody.nodes.length > 0;
...
`, level);
  ...
  visitObject(ast, level, getType=true) {
    ...
    if (!hasExpandField) {
      // Check if this looks like OpenApiRequest based on any of its characteristic fields
      const fieldNames = ast.fields.map(f => f.type === 'objectField' ? _name(f.fieldName) : null).filter(n => n);
      const isOpenApiRequest = fieldNames.some(name => ['headers', 'query', 'body', 'stream'].includes(name));
      
      if (isOpenApiRequest) {
        // Output only the fields that exist, in order: headers (if exists), query, body (or stream)
        const outputFields = [];
        
        // Include headers first if it exists
        const headersField = ast.fields.find(f => f.type === 'objectField' && _name(f.fieldName) === 'headers');
        if (headersField) {
          outputFields.push(headersField.expr);
        }
        
        // Include query if it exists
        const queryField = ast.fields.find(f => f.type === 'objectField' && _name(f.fieldName) === 'query');
        if (queryField) {
          outputFields.push(queryField.expr);
        }
        
        // Include body if it exists (prioritize over stream)
        const bodyField = ast.fields.find(f => f.type === 'objectField' && _name(f.fieldName) === 'body');
        if (bodyField) {
          outputFields.push(bodyField.expr);
        } else {
          // If no body, include stream if it exists
          const streamField = ast.fields.find(f => f.type === 'objectField' && _name(f.fieldName) === 'stream');
          if (streamField) {
            outputFields.push(streamField.expr);
          }
        }
        
        for (let i = 0; i < outputFields.length; i++) {
          this.visitExpr(outputFields[i], level);
      ...
      // For OpenApiRequest, skip the type conversion (.get<...>())
      if(ast.inferred && getType && !isOpenApiRequest) {
  ...
  visitConstructModel(ast, level) {
    ...
    // Always output type name for construct_model
    let isOpenApiRequest = false;
      
    if (ast.object && ast.object.fields) {
      const fieldNames = ast.object.fields.map(f => f.type === 'objectField' ? _name(f.fieldName) : null).filter(n => n);
      isOpenApiRequest = fieldNames.some(name => ['headers', 'query', 'body', 'stream'].includes(name));
    }
  
    if (ast.aliasId.isModule && !isOpenApiRequest) {
      let type = 'Models';
    ...
    if (ast.aliasId.isModel && !isOpenApiRequest) {
    ...
    if (ast.object && ast.object.fields && ast.object.fields.length > 0) {
      if (isOpenApiRequest) {
        // For OpenApiRequest, use fluent setter pattern
        // Need to output the full type name with empty constructor
        this.emit('OpenApiRequest()');
        // Generate fluent setters in order: headers -> query -> body/stream
        const fields = ast.object.fields.filter(f => f.type === 'objectField');
            
        // Sort fields: headers first, then query, then body/stream
        const fieldOrder = { 'headers': 0, 'query': 1, 'body': 2, 'stream': 2 };
        fields.sort((a, b) => {
          const orderA = fieldOrder[_name(a.fieldName)] || 99;
          const orderB = fieldOrder[_name(b.fieldName)] || 99;
          return orderA - orderB;
        });
            
        for (let i = 0; i < fields.length; i++) {
          const field = fields[i];
          const fieldName = _name(field.fieldName);
          const methodName = `set${_upperFirst(fieldName)}`;
          this.emit(`\n${' '.repeat(level * 4)}.${methodName}(`);
          this.visitExpr(field.expr, 0);
          this.emit(')');
        }
      } else {
        // For other types, output normal constructor call
        this.emit('(');
  ...
  visitReturn(ast, level) {
    ...
    if (ast.needCast) {
      // Check if the expectedType is a Response model - use fromMap instead of json().get<>()
      const typeName = _name(ast.expectedType);
      const isResponse = typeName && (typeName.includes('Response') || typeName.endsWith('Response'));
  ...
  visitRuntimeBefore(ast, level) {
    ...
    this.emit(');\n');
    this.emit('\n');
    this.emit('std::exception_ptr _lastExceptionPtr;\n', level);
    ...
    this.emit('});\n', level);
    this.emit(`while (Darabonba::shouldRetry(${RUNTIME}.getRetryOptions(), _context)) {\n`, level);
  ...
  visitRuntimeAfter(ast, level) {
    ...
    this.emit('_lastExceptionPtr = std::current_exception();\n', level + 2);
    ...
    this.emit('}\n', level);
    this.emit('\n');
    this.emit('std::rethrow_exception(_lastExceptionPtr);\n', level);
  }
