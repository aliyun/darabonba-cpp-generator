    ...
    if (ast.catchBlocks && ast.catchBlocks.length > 0) {
      ast.catchBlocks.forEach(catchBlock => {
        ...
        if (!catchBlock.id.type) {
          this.emit(` catch (const Darabonba::Exception ${errorName}) {\n`);
        } else {
...
`, level);
  ...
  visitPropertyAssign(ast, level) {
    ...
    // 检查初始类型是否是系统内置类型
    const isBuiltinModel = this.isSystemBuiltinType(ast.left.id.inferred);
    ...
    for (let i = 0; i < ast.left.propertyPath.length - 1; i++) {
      ...
      if(expr === RESPONSE) {
        expr += `->${name}()`;
      }else if(expr === REQUEST || expr === RUNTIME || isBuiltinModel){
        // REQUEST、RUNTIME 和系统内置类型，直接使用成员方法名，不加 get 前缀
        expr += `.${name}()`;
    ...
    if(expr === RESPONSE) {
      ...
      // deal request_.query.xx - 使用 map 下标访问
      expr = `${REQUEST}.query()["${_name(last)}"] = `;
    } else if(expr === `${REQUEST}.headers()`) {
      // deal request_.headers.xx - 使用 map 下标访问
      expr = `${REQUEST}.headers()["${_name(last)}"] = `;
    } else if(current.type === 'model'|| expr === REQUEST || expr === RUNTIME || isBuiltinModel){
      needClose = true;
  ...
  // 判断是否是 Darabonba 系统内置类型（RuntimeOptions、Request、Response等）
  // 这些类型不使用 get 前缀的 getter 方法
  isSystemBuiltinType(inferred) {
    if (!inferred || inferred.type !== 'model') {
      return false;
    }
    
    // 通过 namespace 判断：系统内置类型在 C++ 中都映射到 Darabonba:: 命名空间
    // 如果是外部模块类型，需要通过 moduleName 获取其 namespace
    if (inferred.moduleName) {
      // 外部模块类型，通过 imports 获取其 namespace
      const moduleInfo = this.imports[inferred.moduleName];
      if (moduleInfo && moduleInfo.namespace) {
        // 只有 Darabonba 命名空间下的类型才不使用 get 前缀
        return moduleInfo.namespace === 'Darabonba' || moduleInfo.namespace.startsWith('Darabonba::');
      }
      return false;
    }
    
    // 对于非模块类型，检查 name 是否是系统内置类型
    // $RuntimeOptions, $Request, $Response 这些都会映射到 Darabonba:: 命名空间
    if (inferred.name && inferred.name.startsWith('$')) {
      // 系统内置类型名称以 $ 开头，且映射到 Darabonba:: 命名空间
      return true;
    }
  ...
  visitPropertyAccess(ast) {
    ...
    var current = ast.id.inferred;
    // 检查初始类型是否是系统内置类型
    const isBuiltinModel = this.isSystemBuiltinType(ast.id.inferred);
    for (var i = 0; i < ast.propertyPath.length; i++) {
      var name = _name(ast.propertyPath[i]);
      // 检查属性名中是否包含特殊字符（如 content-type），必须使用下标访问
      ...
      }else if((expr === REQUEST || expr === RUNTIME || (isBuiltinModel && isFirstIteration))){
        // REQUEST、RUNTIME 和系统内置类型（仅第一次访问），直接使用成员方法名，不加 get 前缀
        // 但如果属性名包含特殊字符，必须使用下标访问
        if (hasSpecialChars) {
          expr += `()["${name}"]`;
        } else {
          expr += `.${name}()`;
  ...
  visitMapAccess(ast, level, assign = false) {
    ...
    if (ast.propertyPath && ast.propertyPath.length) {
      ...
      for (var i = 0; i < ast.propertyPath.length; i++) {
        var name = _name(ast.propertyPath[i]);
        if (expr === REQUEST || expr === RUNTIME || isBuiltinModel) {
          // REQUEST、RUNTIME 和系统内置类型，直接使用成员方法名，不加 get 前缀
          expr += `.${name}()`;
        } else if (current.type === 'model') {
          // 所有 getter 方法都使用 get 前缀
  ...
  visitRuntimeBefore(ast, level) {
    ...
    this.emit('Darabonba::Exception _lastException;\n', level);
    ...
    this.emit('});\n', level);
    this.emit(`while (Darabonba::allowRetry(${RUNTIME}.retryOptions(), _context)) {\n`, level);
    this.emit('if (_retriesAttempted > 0) {\n', level + 1);
    this.emit(`int _backoffTime = Darabonba::getBackoffTime(${RUNTIME}.retryOptions(), _context);\n`, level + 2);
  ...
  visitRuntimeAfter(ast, level) {
    this.emit('} catch (const Darabonba::Exception& ex) {\n', level + 1);
    ...
    this.emit('}\n', level);
    this.emit('\n');
    this.emit('throw *_context.exception();\n', level);
  }
