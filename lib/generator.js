...
`, level);
  ...
  visitObject(ast, level, getType=true) {
    ...
    if (!hasExpandField) {
      // Check if this looks like OpenApiRequest based on query field
      const fieldNames = ast.fields.map(f => f.type === 'objectField' ? _name(f.fieldName) : null).filter(n => n);
      const isOpenApiRequest = fieldNames.includes('query');
      
      if (isOpenApiRequest) {
        // Output only the fields that exist, in order: query, body (or stream)
        const outputFields = [];
  ...
  visitConstructModel(ast, level) {
    ...
    // Always output type name for construct_model (except for special OpenApiRequest case)
    let shouldSkipTypeName = false;
    ...
    if (ast.object && ast.object.fields) {
      ...
      hasQueryField = fieldNames.includes('query');
      shouldSkipTypeName = hasQueryField; // Skip type name for OpenApiRequest direct initialization
    }

    if (ast.aliasId.isModule && !shouldSkipTypeName) {
      let type = 'Models';
    ...
    if (ast.aliasId.isModel && !shouldSkipTypeName) {
    ...
    if (ast.object && ast.object.fields && ast.object.fields.length > 0) {
      if (hasQueryField) {
        // For OpenApiRequest direct initialization (called from visitDeclare), just output the parameters
        this.visitObject(ast.object, level);
      } else {
        // For other types, visitDeclare already outputs 'Type var = ', so we need to output 'Type(...)'
        // But visitConstructModel already output the type name, so just output '(' + object + ')'
        this.emit('(');
        this.visitObject(ast.object, level);
        this.emit(')');
      }
  ...
  visitDeclare(ast, level) {
    ...
    if (isBaseType && isNullValue) {
      ...
      // Check if this is OpenApiRequest construct_model for direct initialization
      let useDirectInit = false;
      if (ast.expr.type === 'construct_model' && ast.expr.object && ast.expr.object.fields) {
        const fieldNames = ast.expr.object.fields.map(f => f.type === 'objectField' ? _name(f.fieldName) : null).filter(n => n);
        useDirectInit = fieldNames.includes('query'); // Only check for query field
      }
