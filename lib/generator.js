...
class Visitor {
  ...
  visitModel(ast, level) {
    assert.equal(ast.type, 'model');
    const modelName = _name(ast.modelName);
    this.emit(`${this.getSubModelClassName(modelName)}::${this.getSubModelClassName(modelName)}(const json &map) : Darabonba::Model() {\n`, level);
    this.emit('validate(map);\n', level + 1);
    this.emit('from_json(map, *this);\n', level + 1);
    this.emit('}\n\n', level);
    this.emit(`void ${this.getSubModelClassName(modelName)}::validate(const json &map) {\n`, level);
  ...
  visitModelToJson(ast, level) {
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      this.emit(`DARABONBA_JSON_TO(${realName}, ${_name(node.fieldName)})\n`, level + 1);
      // this.emit(`if(p._${_name(node.fieldName)}){\n`, level + 1);
  ...
  visitModelValidate(ast, modelName, level) {
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (required || maxLength > 0 || minLength > 0 || maximum > 0 || pattern !== '') {
        if (required) {
          this.emit(`Darabonba::Model::validateRequired("${realName}", map);\n`, level + 1);
        }
        if (pattern !== '') {
          this.emit(`Darabonba::Model::validatePattern("${realName}", map, ${pattern});\n`, level + 1);
        }
        if (maxLength > 0 && maxLength <= 2147483647) {
          this.emit(`Darabonba::Model::validateMaxLength("${realName}", map, ${maxLength});\n`, level + 1);
        }
        if (minLength > 0 && minLength <= 2147483647) {
          this.emit(`Darabonba::Model::validateMinLength("${realName}", map, ${minLength});\n`, level + 1);
        }
        // 不能超过JS中最大安全整数
        if (maximum > 0 && maximum <= Number.MAX_SAFE_INTEGER) {
          this.emit(`${(this.visitFieldType(value, node, modelName))} ${_name(node.fieldName)}Max = ${maximum};`);
          this.emit(`Darabonba::Model::validateMaximum<${this.visitFieldType(value, node, modelName)}>("${realName}", map, ${_name(node.fieldName)}Max);\n`, level + 1);
        }
        // 不能超过JS中最大安全整数
        if (minimum > 0 && minimum <= Number.MAX_SAFE_INTEGER) {
          this.emit(`${this.visitFieldType(value, node, modelName)} ${_name(node.fieldName)}Min = ${minimum};`);
          this.emit(`Darabonba::Model::validateMinimum<${this.visitFieldType(value, node, modelName)}>("${realName}", map, ${_name(node.fieldName)}Min);\n`, level + 1);
        }
      }
    }
  }
  ...
  visitIncludeModelField(ast, modelName, level) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      this.visitFieldType(value, node, modelName);
      this.emit('>');
      this.emit(` _${_name(node.fieldName)};\n`);
      this.emit('\n');
    }
  }
