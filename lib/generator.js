...
class Visitor {
  ...
  visitIncludeModels(models, level){
    ...
    for (let i = 0; i < models.length; i++) {
      ...
      const filename = path.join('include', _dirname(this.modelHeader), `${_camelCase(_snakeCase(modelName))}.hpp`);
      this.emitter = new Emitter(this.outputDir, filename);
  ...
  visitModelsIndex(models) {
    ...
    for (let i = 0; i < sortedModels.length; i++) {
      ...
      this.headers.push(`${this.modelHeader}/${_camelCase(_snakeCase(modelName))}.hpp`);
    }
  ...
  visitFieldsToJson(ast, level) {
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if(this.checkStream(value) || (value.fieldType && value.fieldType.idType === 'module')) {
  ...
  visitIncludeModelFunc(ast, modelName, level) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      let attrGetName = _avoidKeywords(attrName);
      let setterParamName = attrGetName;  // setter参数名
      let useGetPrefix = false;  // 是否使用get前缀
      
      // 对于嵌套模型类型的字段，参数名使用小写开头避免与类名冲突
      ...
      if (attrName.startsWith('has') && attrName.length > 3 && attrName[3] >= 'A' && attrName[3] <= 'Z') {
        useGetPrefix = true;  // 对于hasXxx字段，使用getHasXxx()而不是hasXxx()
        setterParamName = attrName + 'Value';  // setter参数名使用Value后缀避免冲突
      } else if (isNestedModel) {
        // 对于嵌套模型，getter方法名和setter参数名都使用小写开头避免与类名冲突
        attrGetName = attrName.charAt(0).toLowerCase() + attrName.slice(1);
        setterParamName = attrGetName;
      }
      // get method
      this.emit('inline ', level);
      if(value.fieldType && _isBaseType(value.fieldType)) {
        this.visitFieldType(value, level, modelName, attrName);
        const getterName = useGetPrefix ? `get${_upperFirst(attrName)}` : attrGetName;
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if((value.fieldType && (value.fieldType === 'any' || value.fieldType.type === 'any'))
       || ((value.type === 'map' || value.fieldType === 'map') && _name(value.valueType) === 'any')) {
        ...
        this.emit('', level);
        this.visitFieldType(value, level, modelName, attrName);
        this.emit(` ${attrName}_ = nullptr;\n`);
      } else {
        this.emit('std::shared_ptr<', level);
        this.visitFieldType(value, level, modelName, attrName);
        this.emit(`> ${attrName}_ = nullptr;\n`);
      }
    }
  ...
  visitYield(ast, level) {
    ...
    this.emit(';\n');
    const retType = this.isAsyncFunction ? 'Darbaonba::FutureGenerator' : 'Darbaonba::Generator';
