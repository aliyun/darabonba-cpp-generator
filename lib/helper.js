...
const keywords = [
  ...
  'xor', 'xor_eq',
  // 宏内部使用的标识符，需要避免冲突
  'Type' // DARABONBA_PTR_FROM_JSON 宏内部使用的类型别名
];
...
/**
 * 获取字段的 getter 方法名（参考 Java SDK 规范）
 * 所有字段都添加 get 前缀
 * @param {string} fieldName - 字段名
 * @returns {string} getter 方法名
 */
function _getGetterName(fieldName) {
  return 'get' + _upperFirst(fieldName);
}
...
function _defaultValue(name){
  ...
  console.log(name);
  throw new Error('unimplemented: unknown type "' + name + '"');
}
...
module.exports = {
  ...
  _camelCase, _isBuiltinModel, MODEL, _avoidKeywords, _getGetterName, _getInclude, _namespace, _dirname
};
